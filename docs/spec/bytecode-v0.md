...
- ноль или больше user/std skills как functions;
...
Снимает `argc` arguments со стека, вызывает function/protocol/std call, кладёт результат.
...
Часть `core.*` понижается в S std skills.
