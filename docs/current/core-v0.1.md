...
```text
core API -> S implementation -> minimal host primitive
```

Неправильное направление:

```text
core API -> renamed host/Racket implementation
```
...
| `core.file.read_text(path)` | implemented | S core module over `host.file.read` | yes, через canonical |
| `core.file.write_text(path, text)` | implemented | S core module over `host.file.write` | yes |
| `core.json.encode(value)` | implemented | S core module over `host.json.encode` | yes |
| `core.str.lines_count(text)` | implemented | S algorithm over `core.str.len/at` | yes |
| `core.str.lines(text)` | implemented | S algorithm over `core.str.split` | yes |
| `core.str.len(text)` | implemented | S core module over `host.str.len` | yes |
| `core.str.at(text, index)` | implemented | S core module over `host.str.at` primitive | yes |
| `core.str.slice(text, start, end)` | implemented | S core module over `host.str.slice` primitive | yes |
| `core.str.join(...)` | implemented | `host.str.join` bridge | yes |
| `core.str.add(left, right)` | implemented | S core module over `host.str.join` | yes |
| `core.str.eq(left, right)` | implemented | S core module over `host.str.eq` | yes |
...
| `core.str.trim(text)` | implemented | S core module over `host.str.trim` | yes |
| `core.str.upper(text)` | implemented | S core module over `host.str.upper` | yes |
| `core.str.lower(text)` | implemented | S core module over `host.str.lower` | yes |
| `core.str.split(text, separator)` | implemented | S algorithm over `len/at/slice/group.append` | yes |
| `core.num.parse(text)` | implemented | S core module over `host.math.parse` | yes |
| `core.num.abs(value)` | implemented | S core module over `host.math.abs` | yes |
| `core.num.min(...)` | implemented | `host.math.min` bridge | yes |
| `core.num.max(...)` | implemented | `host.math.max` bridge | yes |
| `core.num.round(value)` | implemented | S core module over `host.math.round` | yes |
| `core.group.count(group)` | implemented | runtime native | yes |
| `core.group.at(group, index)` | implemented | runtime native | yes |
| `core.group.append(group, value)` | implemented | runtime native storage primitive | yes |
...
- `core.str.lines` и `core.str.split` уже являются S-алгоритмами поверх низкоуровневых string/group primitives.
...
- CLI framework;
- logging framework;
- collection operations beyond `core.group.count`, `core.group.at`, and `core.group.append`;
- package manager;
