...
```text
Saltic-owned frontend/runtime:  55–65%
LLVM backend:                  25–30%
Complete cycle without Racket: 28–32%
```

The current working estimate toward honest self-hosting is **about 30%**.
...
structural regression: 28/28
string regression:      8/8
native integration:    28/28
```
...
- heterogeneous and dynamically growing Groups;
- heterogeneous `Box` fields and a general native value representation;
