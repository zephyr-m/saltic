...
Это не стандартная библиотека S и не финальный дизайн языка. Это честное имя для операций, которые сейчас выполняются host-средой, чтобы ранние программы на S могли делать полезные вещи до появления module/import системы и настоящей `core`.

Общая карта слоёв описана в [Библиотечной архитектуре](../start/libraries.md).
Минимальный канон будущей `core` описан в [core v0.1](../current/core-v0.1.md).
...
Обычные пользовательские и canonical examples должны идти через `use core` и `core.*`.

Если возможность уже выглядит как обычный пользовательский API, её место в `core`, а не в `host`.
