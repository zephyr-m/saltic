...
node js/1-parser.js s2/core/tests/fixed.saltic >"$work/ast/fixed-types.json"
echo "фиксированные типы: проверяю рабочую программу"
node js/2-checker.js s2/core/tests/fixed.saltic >"$work/checker/fixed-types.json"

echo "фиксированные типы: разбираю программу ошибок"
node js/1-parser.js s2/core/tests/fixed-errors.saltic >"$work/ast/fixed-types-errors.json"
echo "фиксированные типы: сохраняю диагностику программы ошибок"
node tests/checker-parity.js expected s2/core/tests/fixed-errors.saltic \
    >"$work/checker/fixed-types-errors.txt"
sed -E 's/^([^|]+)\|[0-9]+\|[0-9]+\|/\1|/' \
    "$work/checker/fixed-types-errors.txt" \
    >"$work/checker/fixed-types-errors-normalized.txt"
if ! diff -u s2/core/tests/expected/fixed-errors.txt \
    "$work/checker/fixed-types-errors-normalized.txt" \
...
node js/4-vm.js "$work/bootstrap/checker.elf" --steps 1000000000 -- \
    s2/core/tests/fixed.saltic >"$work/checker/saltic-valid.txt"
test ! -s "$work/checker/saltic-valid.txt"
node js/4-vm.js "$work/bootstrap/checker.elf" --steps 1000000000 -- \
    s2/core/tests/fixed-errors.saltic >"$work/checker/saltic-errors.txt"
...
nix-shell -p pkgsCross.riscv32-embedded.buildPackages.gcc --run \
    "node js/3-compiler.js --assembly '$work/compiler/js.s' s2/core/tests/fixed.saltic '$work/compiler/js.elf'"
...
node js/4-vm.js "$work/bootstrap/compiler.elf" --steps 1000000000 -- \
    s2/core/tests/fixed.saltic "$work/compiler/saltic.s"
if ! diff -u \
    -I '^  lw a0, 0(sp)$' \
    -I '^  addi a1, sp, 4$' \
...
if ! diff -u s2/core/tests/expected/fixed.txt "$work/run/js.txt" \
