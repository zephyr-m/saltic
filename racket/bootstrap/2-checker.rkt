...
(module+ main
  ...
  (when (not (or (= (vector-length args) 1)
    ...
    (eprintf "usage: racket racket/bootstrap/2-checker.rkt [--json] <file.s>\n")
    (exit 2))
  (define json-mode?
    (and (= (vector-length args) 2)
         (equal? (vector-ref args 0) "--json")))
  ...
  (with-handlers ([exn:fail?
    ...
    (cond
      [json-mode?
       ...
       (unless (diagnostics-empty? diagnostics) (exit 1))]
      [(diagnostics-empty? diagnostics) (displayln "ok")]
      [else
