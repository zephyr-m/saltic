...
function globals(items){
  ...
  const put=(map,n,v)=>exists(n)?ds.push(diagnostic(null,`duplicate top-level name '${n}'`)):map.set(n,v)
  for(const raw of items){
    ...
    if(n[0]==='use'){
      ...
      if(aliases.has(alias)&&aliases.get(alias)!==target)ds.push(diagnostic(null,`duplicate module alias '${alias}'`));else aliases.set(alias,target)
    ...
    else if(n[0]==='entry'&&!module.length){if(g.entry)ds.push(diagnostic(null,'duplicate program entry'));else g.entry=[n[1],n[2]]}
  }
  return[g,ds]
}
...
function pathType(parts,env,g,loc){
  ...
  if(parts.length>1){const enumName=resolved(g.enums,parts.slice(0,-1),env,g);if(enumName)return g.enums.get(enumName).includes(parts.at(-1))?result(['enum',enumName]):result('unknown',[diagnostic(loc,`enum '${enumName}' has no variant '${parts.at(-1)}'`)])}
  const [head,...tail]=parts
  if(['host','world','visual','ui'].includes(head))return result('module-path')
  if(head==='core')return core(g)?result('module-path'):result('unknown',[diagnostic(loc,"module 'core' is not imported; add 'use core'")])
  if(head==='error')return result('error')
  const t=env.get(head);return t?fieldPath(t,tail,g,loc):parts.length===1?result('unknown',[diagnostic(loc,`unknown name '${head}'`)]):result('unknown')
}
const calls={
...
function expression(raw,env,g){const loc=place(raw),n=strip(raw),tag=n[0];if(tag==='number')return result('number');if(tag==='string')return result('string');if(tag==='answer')return result('answer');if(tag==='none')return result('none');if(tag==='enum-value')return result('enum-value');if(tag==='path')return pathType(n.slice(1),env,g,loc);if(tag==='group'){const rs=n.slice(1).map(x=>expression(x,env,g));return result(['group',rs.reduce((t,x)=>merge(t,x.type),'unknown')],rs.flatMap(x=>x.diagnostics))}if(tag==='box-new')return boxNew(n[1],n.slice(2),env,g,loc);if(tag==='call')return callExpression(n[1],n.slice(2),env,g,loc);if(tag==='binary'){const l=expression(n[2],env,g),r=expression(n[3],env,g),ds=[...l.diagnostics,...r.diagnostics],op=n[1];if(['+','-','*','/','>','<'].includes(op)&&l.type!=='unknown'&&r.type!=='unknown'){
  ...
  } else if(l.type!=='number'||r.type!=='number')ds.push(diagnostic(null,`operator '${op}' expects numbers, got ${typeText(l.type)} and ${typeText(r.type)}`))
...
function boxNew(name,fields,env,g,loc){const box=resolved(g.boxes,name.split('.'),env,g);if(!box)return result('unknown',[diagnostic(loc,`unknown Box '${name}'`)]);const model=g.boxes.get(box),expected=new Map(model.map(f=>[strip(f)[1],fieldLiteral(strip(f)[2],box,g)])),seen=new Set,ds=[];for(const f0 of fields){const f=strip(f0),key=f[1],r=expression(f[2],env,g);ds.unshift(...r.diagnostics);if(seen.has(key))ds.unshift(diagnostic(loc,`duplicate field '${key}' in Box literal '${name}'`));seen.add(key);if(!expected.has(key))ds.unshift(diagnostic(loc,`Box '${name}' has no field '${key}'`));else if(!compatible(expected.get(key),r.type))ds.unshift(diagnostic(loc,`cannot assign ${typeText(r.type)} to field '${key}' of Box '${name}' with type ${typeText(expected.get(key))}`))}return result(['box',box],ds.reverse())}
...
function statement(raw,env,g,inSkill){const loc=place(raw),n=strip(raw),tag=n[0];if(tag==='var'){const r=expression(n[2],env,g),ds=[...r.diagnostics];if(env.vars.has(n[1]))ds.push(diagnostic(loc,`variable '${n[1]}' is already declared in this scope`));else if(top(g,n[1]))ds.push(diagnostic(loc,`variable '${n[1]}' conflicts with top-level name`));else env.vars.set(n[1],r.type);return ds}if(tag==='assign'){const r=expression(n[2],env,g),ds=[...r.diagnostics],name=n[1];if(g.constants.has(name))ds.push(diagnostic(loc,`cannot assign to constant '${name}'`));else if(top(g,name))ds.push(diagnostic(loc,`cannot assign to top-level name '${name}'`));else{const old=env.get(name);if(!old)ds.push(diagnostic(loc,`variable '${name}' is not declared`));else if(compatible(old,r.type))env.set(name,merge(old,r.type));else ds.push(diagnostic(loc,fixedTypes.has(old)||fixedTypes.has(r.type)?`нельзя присвоить ${typeText(r.type)} переменной '${name}' типа ${typeText(old)}`:`cannot assign ${typeText(r.type)} to variable '${name}' of type ${typeText(old)}`))}return ds}if(tag==='field-assign'){const parts=strip(n[1]).slice(1),base=pathType(parts.slice(0,-1),env,g,loc),value=expression(n[2],env,g),ds=[...base.diagnostics,...value.diagnostics];if(Array.isArray(base.type)&&base.type[0]==='box'){const expected=fieldType(base.type[1],parts.at(-1),g);if(!expected)ds.push(diagnostic(loc,`Box '${base.type[1]}' has no field '${parts.at(-1)}'`));else if(!compatible(expected,value.type))ds.push(diagnostic(loc,`cannot assign ${typeText(value.type)} to field '${parts.at(-1)}' of Box '${base.type[1]}'`))}return ds}if(tag==='out')return[...(!inSkill?[diagnostic(loc,"'out' can only be used inside skill")]:[]),...expression(n[1],env,g).diagnostics];if(tag==='expr')return expression(n[1],env,g).diagnostics;if(tag==='if')return[...expression(n[1],env,g).diagnostics,...block(n[2],new Scope(env),g,inSkill)];if(tag==='switch')return[...expression(n[1],env,g).diagnostics,...n.slice(2).flatMap(c=>expression(strip(c)[2],env,g).diagnostics)];if(tag==='drum')return[...expression(n[1],env,g).diagnostics,...block(n[2],new Scope(env),g,inSkill)];return[diagnostic(loc,`unsupported statement ${JSON.stringify(n)}`)]}
function checkDatum(ast){const n=strip(ast);if(n[0]!=='program')return[diagnostic(null,'expected program AST')];const [g,start]=globals(n.slice(1)),ds=[...start];for(const raw of n.slice(1)){const x=strip(raw),module=parser.moduleOf(raw)??[],scope=()=>{const env=new Scope;env.module=module;return env};if(x[0]==='const')ds.push(...expression(x[2],scope(),g).diagnostics);else if(x[0]==='box'){const seen=new Set,methods=new Set,fields=x.slice(2).map(strip).filter(f=>f[0]==='field');for(const f of fields){if(seen.has(f[1]))ds.push(diagnostic(null,`duplicate field '${f[1]}' in Box '${x[1]}'`));seen.add(f[1]);ds.push(...expression(f[2],scope(),g).diagnostics)}for(const method of x.slice(2).map(strip).filter(f=>f[0]==='subject-skill')){if(methods.has(method[1]))ds.push(diagnostic(null,`duplicate skill '${method[1]}' in Box '${x[1]}'`));methods.add(method[1]);const env=scope();env.subject=qualified(module,x[1]);for(const f of fields)env.vars.set(f[1],fieldLiteral(f[2],qualified(module,x[1]),g));for(const p of method[2])env.vars.has(p)?ds.push(diagnostic(null,`duplicate parameter '${p}'`)):env.vars.set(p,'unknown');ds.push(...block(method[3],env,g,true))}}else if(x[0]==='entry'||x[0]==='skill'){const env=scope();for(const p of x[0]==='entry'?x[1]:x[2])env.vars.has(p)?ds.push(diagnostic(null,`duplicate parameter '${p}'`)):env.vars.set(p,'unknown');ds.push(...block(x[0]==='entry'?x[2]:x[3],env,g,true))}}return ds}
