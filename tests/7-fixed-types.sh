...
node js/1-parser.js tests/fixed-types.s >"$work/ast/fixed-types.json"
echo "фиксированные типы: проверяю рабочую программу"
node js/2-checker.js tests/fixed-types.s >"$work/checker/fixed-types.json"

echo "фиксированные типы: разбираю программу ошибок"
node js/1-parser.js tests/fixed-types-errors.s >"$work/ast/fixed-types-errors.json"
echo "фиксированные типы: сохраняю диагностику программы ошибок"
node tests/checker-parity.js expected tests/fixed-types-errors.s \
...
node tests/checker-parity.js build "$work/bootstrap/checker.s"
nix-shell -p pkgsCross.riscv32-embedded.buildPackages.gcc --run \
    "node js/3-compiler.js '$work/bootstrap/checker.s' '$work/bootstrap/checker.elf'"
node js/4-vm.js "$work/bootstrap/checker.elf" --steps 1000000000 -- \
    tests/fixed-types.s >"$work/checker/saltic-valid.txt"
test ! -s "$work/checker/saltic-valid.txt"
node js/4-vm.js "$work/bootstrap/checker.elf" --steps 1000000000 -- \
    tests/fixed-types-errors.s >"$work/checker/saltic-errors.txt"
...
nix-shell -p pkgsCross.riscv32-embedded.buildPackages.gcc --run \
    "node js/3-compiler.js --assembly '$work/compiler/js.s' tests/fixed-types.s '$work/compiler/js.elf'"

echo "фиксированные типы: собираю Saltic-компилятор"
node tests/compiler-parity.js build "$work/bootstrap/compiler.s"
nix-shell -p pkgsCross.riscv32-embedded.buildPackages.gcc --run \
    "node js/3-compiler.js --assembly '$work/bootstrap/compiler-small.s' '$work/bootstrap/compiler.s' '$work/bootstrap/compiler-small.elf'"
...
node js/4-vm.js "$work/bootstrap/compiler.elf" --steps 1000000000 -- \
    tests/fixed-types.s "$work/compiler/saltic.s"
