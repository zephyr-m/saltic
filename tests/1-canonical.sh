...
cd "$root"
node js/1-parser.js canonical.saltic >/dev/null
node js/2-checker.js canonical.saltic >/dev/null
nix-shell -p pkgsCross.riscv32-embedded.buildPackages.gcc \
    --run "node js/3-compiler.js canonical.saltic '$work/canonical.elf'"
