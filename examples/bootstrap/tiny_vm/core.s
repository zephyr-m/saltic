...
BoundaryTable = Box {
    first = BoundaryHandler {}
    second = BoundaryHandler {}
    count = 0
}
...
skill boundary_table_one(first) {
    out BoundaryTable {
        first = first
        count = 1
    }
...
skill std_boundary_table() {
    out boundary_table_one(boundary_handler("core.io.println"))
}
...
skill boundary_find(boundaries, name) {
    (boundaries.count > 0) {
        (boundaries.first.name == name) {
            out boundaries.first
        }
    }
    (boundaries.count > 1) {
        (boundaries.second.name == name) {
            out boundaries.second
        }
    }
    out BoundaryHandler {
        name = "__missing__"
