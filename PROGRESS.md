...
- [x] Блок 6: stage-1 собирается с `SALTIC_DIRECT_ONLY=1`; bootstrap и refresh требуют исполняемый ELF и отсутствие `.s`.
- [x] Блок 6, stage-1 подтверждён: `just test bootstrap`, `bootstrap: ok`.
- [x] Блок 6: три стадии bootstrap идут с заглушками GNU `as`/`gcc` и проверкой отсутствия `.s`.
- [x] Блок 6, GNU: `just test bootstrap` подтвердил отсутствие `.s`, `as` и `gcc` на трёх стадиях, `bootstrap: ok`.
- [x] Блок 6, VM: stage-2 собирает compiler с кучей 64 МиБ; Saltic VM исполняет его на `program() { out 7 }` и ждёт ELF.
- [x] Блок 6, VM: загрузчик получил кучу 4 ГиБ; на 512 МиБ он завершался с кодом 65 до записи отчёта.
- [x] Блок 6, VM: `vm.run` возвращает результат сразу после `halted`; лимит шагов 251658240, ниже максимума number.
- [x] Блок 6, VM: длинный прогон пишет «vm: старт» и шаг раз в 1048576.
- [x] Блок 6, VM: шаг прогресса уменьшен до 100000.
- [x] Блок 6: toolchain пишет фазы «читаю», «проверяю», «собираю», «пишу».
- [x] Блок 6, VM: до старта пишет чтение, загрузку образа и стек; счётчик шагов начинается с нуля.
- [x] Блок 6, VM: журнал шагов помечается секундами с предыдущей строки.
- [x] Блок 6 закрыт: VM-проверка подтверждена владельцем, больше 300000 инструкций без неожиданного trap. Восстановление — `compiler.elf` из Git. Assembly/GNU rescue удаляется в пункте 7.
- [x] Блок 7: добавлен `just test closure`, путь source → прямой ELF → QEMU для канона.
- [x] Блок 7, closure подтверждён: `just test closure`, `closure: ok`.
